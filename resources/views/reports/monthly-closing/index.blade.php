<x-app-layout>
    ...
    if (!window.monthlyClosingScriptExecuted) {
        ...
        function initializeCharts() {
            ...
            @php
                $bookerRevenueData = ($reportData['booker_data'] ?? collect([]))->map(function($item) {
                    return [
                        'label' => $item['booker']->name,
                        'revenue' => (float) $item['total_cache_brl'],
                        'booker_commission' => (float) $item['total_booker_commission'],
                        'agency_commission' => (float) $item['total_agency_commission'],
                        'net_value' => (float) $item['net_value'],
                        'color' => '#' . str_pad(dechex(mt_rand(0, 0x999999)), 6, '0', STR_PAD_LEFT)
                    ];
                });
            @endphp
            ...
            if (revenueCtx) {
                ...
                if (bookerRevenueData && bookerRevenueData.length > 0) {
                    // Esconde a mensagem de nenhum dado
                            ...
                            datasets: [
                                {
                                    ...
                                    data: bookerRevenueData.map(item => item.revenue),
                                    backgroundColor: bookerRevenueData.map(item => item.color),
                                    borderColor: bookerRevenueData.map(item => item.color),
                                    borderWidth: 1,
                                    borderRadius: 4,
                                    barThickness: 'flex',
                                    barPercentage: 0.8,
                                ...
                                },
                                {
                                    ...
                                    data: bookerRevenueData.map(item => item.booker_commission),
                                    backgroundColor: 'rgba(147, 51, 234, 0.7)', // Roxo com transparência
                                ...
                                },
                                {
                                    ...
                                    data: bookerRevenueData.map(item => item.agency_commission),
                                    backgroundColor: 'rgba(99, 102, 241, 0.7)', // Índigo com transparência
                                ...
                                {
                                    label: 'Valor Líquido',
                                    data: bookerRevenueData.map(item => item.net_value),
                                    backgroundColor: 'rgba(16, 185, 129, 0.7)', // Verde com transparência
                    ...
                    // Mostra a mensagem de nenhum dado disponível
                    if (noRevenueData) noRevenueData.classList.remove('hidden');
                    
                    // Garante que o canvas está vazio
                    if (revenueCtx) {
            ...
            // Configuração do gráfico de pizza de distribuição por booker
            @php
                $bookerPieData = ($reportData['booker_data'] ?? collect([]))->map(function($item) {
                    return [
                        'label' => $item['booker']->name,
                        'value' => (float) $item['total_cache_brl'],
            ...
            if (pieCtx) {
                ...
                if (bookerPieData && bookerPieData.length > 0) {
                    // Esconde a mensagem de nenhum dado
                    if (noPieData) noPieData.classList.add('hidden');
                    
                    // Cria o gráfico de pizza e armazena na variável global
                    window.pieChart = new Chart(pieCtx, {
                        type: 'pie',
                        data: {
                            labels: bookerPieData.map(item => item.label),
                            datasets: [{
                                data: bookerPieData.map(item => item.value),
                                backgroundColor: bookerPieData.map(item => item.color),
                                borderWidth: 1,
                                borderColor: '#fff',
                                hoverOffset: 4
                            }]
                    ...
                    // Mostra a mensagem de nenhum dado disponível
                    if (noPieData) noPieData.classList.remove('hidden');
                }
            }
        }
        ...
        if (document.readyState === 'complete' || document.readyState === 'interactive') {
            // O DOM já está pronto, inicializa os gráficos
            console.log('DOM já carregado, inicializando gráficos...');
            initializeCharts();
        } else {
            // Espera o DOM carregar primeiro
            document.addEventListener('DOMContentLoaded', function() {
                console.log('DOM totalmente carregado, inicializando gráficos...');
                initializeCharts();
            });
        }
    } // Fecha a verificação de script já executado
    </script>
    @endpush
</x-app-layout>