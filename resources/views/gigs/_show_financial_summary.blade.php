...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden">
    ...
    <div class="p-6 space-y-3 text-sm">
        <div>
            ...
            <span class="font-semibold text-gray-900 dark:text-white ml-2">{{ $gig->currency }} {{ number_format($gig->cache_value ?? 0, 2, ',', '.') }}</span>
        </div>
        <div>
            <span class="text-gray-500 dark:text-gray-400">Total Recebido:</span>
            <span class="font-semibold text-green-600 dark:text-green-400 ml-2">{{ $gig->currency }} {{ number_format($financialData['totalReceivedInOriginalCurrency'], 2, ',', '.') }}</span>
        </div>
        <div>
            <span class="text-gray-500 dark:text-gray-400">Saldo Pendente:</span>
            <span class="font-semibold {{ ($financialData['pendingBalanceInOriginalCurrency'] ?? 0) <= 0.01 ? 'text-gray-500 dark:text-gray-400' : 'text-red-600 dark:text-red-400' }} ml-2">{{ $gig->currency }} {{ number_format($financialData['pendingBalanceInOriginalCurrency'], 2, ',', '.') }}</span>
        </div>
